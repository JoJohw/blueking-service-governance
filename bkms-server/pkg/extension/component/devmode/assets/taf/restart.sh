#!/bin/bash
# restart.sh - taf 版二进制热更新：MD5 校验 → 替换二进制 → SIGUSR2 重启
# 用法: restart.sh <random_name> <md5>
# taf 无法提前知道 bin 路径，需要在运行时通过 SERVER_NAME_ENV 动态确定
...
# 清理旧的应用二进制
cleanup_old_binary() {
    local old_binary="${TAF_BIN_PATH}/${SERVER_NAME}"
...
move_new_binary() {
    ...
    local target_file="${TAF_BIN_PATH}/${SERVER_NAME}"
    
    # 确保目标目录存在
    mkdir -p "${TAF_BIN_PATH}"
...
wait_old_process_exit() {
    ...
    for ((i=1; i<=${timeout}; i++)); do
        ...
        # 检查是否有新进程已经启动
        local pid_count=$(get_process_count)
...
verify_process_started() {
    ...
    for ((i=1; i<=${VERIFY_RETRY_COUNT}; i++)); do
        ...
        if [ ${count} -ge 1 ]; then
            # 获取所有运行中的进程 PID
            local all_pids=$(get_process_pids)
            local new_pid=""
...
main() {
    log_info "=========================================="
    log_info "Restart script started (TAF)"
    log_info "=========================================="
    ...
    # 动态获取 taf 服务的二进制路径和名称（从运行中的进程确定）
    if ! get_taf_server_info; then
        log_warn "Could not determine taf server info from running process, will try to start fresh after binary replacement"
    fi
    ...
    log_info "  Server name env: ${SERVER_NAME_ENV}"
    log_info "  TAF bin path: ${TAF_BIN_PATH:-unknown}"
    log_info "  Server name: ${SERVER_NAME:-unknown}"
    ...
    # 4. 替换二进制（必须已确定路径）
    if [ -z "${TAF_BIN_PATH}" ] || [ -z "${SERVER_NAME}" ]; then
        clear_stop_flag
        log_fatal "TAF_BIN_PATH or SERVER_NAME not determined, cannot replace binary"
        exit ${RESTART_FAILED}
    fi
