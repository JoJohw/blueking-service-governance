...
- [x] V = 9（台账预估 4~5，摸底校准：步骤条 2 + 模板选择 2 + tRPC 向导 5）
  - 补录：向导「取消」路径（`trpc/index.vue:194-199` → `router.back()`），按指南 §2.2「取消」二阶风险补齐；
    实测 `useLeaveConfirm()` 未传 formModel → isDirty 恒 false，取消时**不会弹离开确认**，按现状断言
...
- **P2**：重复提交拦截——复核确认「创建」按钮（`trpc/index.vue:60-67`）无 loading/disabled，**当前无保护**；是否增加属产品决策。
- 备注：取消时的「离开确认」实测不触发（`useLeaveConfirm()` 未传 formModel），如需该行为属产品/前端改进项。
