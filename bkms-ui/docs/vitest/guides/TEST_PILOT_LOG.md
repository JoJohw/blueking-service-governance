...
| 2026-09-08 | S9（撤下） | 环境管理列表页表格渲染 **flaky**：同一断言（等待环境名出现）在连续运行中时通过时超时（约 50% 概率）；「新建环境」弹窗在 jsdom 下无法稳定断言打开（弹窗标题未进入可访问树，且打开时依赖空间/APM 拉取） | 疑似 vxe 渲染时序/ResizeObserver 回调时机不稳定（S14 同方案连跑稳定，暂不能定位到公共垫片） | **撤下 S9 用例文件，不交付不稳定用例**；待后续专项排查（建议：增大 waitFor 超时、或在渲染后主动触发一次 resize 回调） | ai_unsure.md + 本表 |
