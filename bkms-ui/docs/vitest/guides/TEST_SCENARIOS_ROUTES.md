...
- 判定分支：模板类型选择（路由级分支）/ 表单校验通过/不通过 / 提交成功/失败 → **V = 4~5**
...
- 判定分支（预估）：部署成功/失败 / 回滚确认/取消 / 历史列表三态 → **V ≈ 4~5**
- 设计方法：基本路径 + 二阶风险（回滚属破坏性操作：当前版本保护、回滚中操作拦截，均推断/需确认）
- 用例草稿（实施前校准）：
  - `it('当用户执行 Helm 部署时，应发起部署并展示部署结果反馈')`
  - `it('当用户预览回滚并确认时，应回滚至目标版本')`
  - `it('当用户取消回滚时，应保持当前版本不变')`
