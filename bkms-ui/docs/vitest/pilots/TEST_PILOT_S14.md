...
- 接口失败时源码有 `.catch()`（置异常态 + 空数据 + 复位 loading），**异常态文案因 vxe 空态插槽未渲染而不可见**，用户看到的是「空表格」而非「数据获取异常 + 刷新」——已结案（N/A）：「失败态卡片在浏览器是否可见」超出 vitest 工具边界（jsdom 无渲染引擎，vxe 空态插槽不生成）；逻辑层已验证（catch → 异常态 + 空态插槽接线正确），渲染层可由真实浏览器人工确认，非必测缺口。
