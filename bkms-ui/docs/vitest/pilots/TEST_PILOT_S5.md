...
| 4 | 4/5 | 源码现状是**回传字符串 `'12'` 而非数字**（`v-model.trim` + bkui-vue number input），严格数值断言不成立 | 改为「数字输入形态」断言（`toHaveAttribute('type','number')`）+ 字符串回传；已复核下游无隐患（接口 `Record<string, unknown>`、消费方接受 string、无数值转换） |
